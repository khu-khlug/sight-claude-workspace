...
- 유효한 문자 발송 요청이 수신자별 결과를 반환하면, 결과가 `SENT`, `FAILED` 또는 `SKIPPED`인지와 관계없이 모든 운영진에게 시스템 알림을 생성한다. 알림은 수신자 목록과 HTML 이스케이프한 메시지 원문을 포함한다.
- SOLAPI 요청 자체가 실패하여 `500 Internal Server Error`를 반환하면 문자 발송 결과를 확정할 수 없으므로 운영진 알림을 생성하지 않는다.
...
- 문자 발송 결과가 반환된 뒤 운영진 시스템 알림을 생성한다. 이 알림 생성은 SOLAPI에 대한 추가 요청을 만들지 않는다.
...
- 문자 발송 이력 테이블은 추가하지 않는다. 다만 문자 발송 결과를 알리는 운영진 시스템 알림에는 수신자 이름 또는 직접 지정 전화번호와 HTML 이스케이프한 메시지 원문이 저장된다. 알림 내용은 운영진 알림 조회 권한 범위에서만 제공한다.
...
- 문자 발송 요청이 수신자별 결과를 반환하면 운영진 전체에 시스템 알림이 저장된다. 따라서 직접 지정 전화번호와 메시지 원문도 알림 데이터로 보존된다.
- SOLAPI 요청 자체가 실패하면 운영진 시스템 알림을 생성하지 않는다.
...
- 문자 발송 결과가 `SENT`, `FAILED` 또는 `SKIPPED`인 요청마다 운영진 전체에 수신자 목록과 메시지 원문을 포함한 시스템 알림이 생성되는지 확인한다.
- SOLAPI 요청 자체가 실패하면 운영진 시스템 알림이 생성되지 않는지 확인한다.
